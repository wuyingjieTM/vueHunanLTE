        ...
        getTabBody(item) {
          ...
          if (_this.text) {
            ...
            return httpUrl.url[this.tabUrl](this.tabParams).then(success => {
              const data = JSON.parse(success.data.body);
              const bodyData = [];
              for (let item of data.tabData) {
                const obj = {};
                _this.tabHead.forEach((element, index) => {
                  if (2 === index) {
                    obj[element["field"]] = "<a>" + item[index] + "</a>";
                  } else if (4 === index) {
                    obj[element["field"]] = "<a>" + item[index] + "</a>";
                  } else if (5 === index && "竞对差" == item[index]) {
                    obj[element["field"]] = "<a>" + item[index] + "</a>";
                  } else if (11 === index && null != item[index]) {
                    obj[element["field"]] = "<a>" + item[index] + "</a>";
                  } else {
                    obj[element["field"]] = item[index];
                  }
                });
                bodyData.push(obj);
              }
